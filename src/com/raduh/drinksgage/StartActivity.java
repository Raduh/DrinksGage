...
public class StartActivity extends Activity {
	...
	public void setBodyData (View parentView){
		...
		if(wgt != -1) {weight.setProgress(wgt-20); weight_txt.setText(" "+wgt+" kg");}
		
		weight.setOnSeekBarChangeListener(new OnSeekBarChangeListener(){

			@Override
			public void onProgressChanged(SeekBar arg0, int progressNew, boolean fromUser) {
		...
		if(isF) female.setChecked(true);
			...
			public void onClick(View v) {
				...
				if(male.isChecked())
					othData.edit().putBoolean("isMale", true).putBoolean("isFemale", false).commit();
				if(female.isChecked())
					othData.edit().putBoolean("isMale", false).putBoolean("isFemale", true).commit();
				
				othData.edit().putInt("userWeight", (weight.getProgress() + 20)).commit();
	...
	public void showResult (View view){
		...
		if((othData.getBoolean("isMale", false) == false) 
				&& (othData.getBoolean("isFemale", false) == false)){
